import { Stagehand, localBrowser, type StagehandBrowser } from "@browserbasehq/stagehand";
import { z } from "zod";
...
async function extractCitations(stagehand: Stagehand): Promise<Citation[]> {
	...
	const extractedLinks = (await page.evaluate(() => {
		...
		return links.map((a) => ({ url: a.href, text: a.innerText })).filter((l) => l.url && l.url.startsWith("http"));
